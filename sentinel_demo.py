...
def render_conversational_tab():
    ...
    query = st.text_input(
        "Ask Sentinel",
        value="Show me all internet-exposed RDS instances with unencrypted data in production accounts",
    )
    ...
    envs = st.multiselect("Environment scope", options=["Production", "Sandbox", "Shared"], default=["Production"])
    ...
    col1, col2, col3 = st.columns(3)
    do_tf = col1.button("Remediate all via Terraform", use_container_width=True)
    do_jira = col2.button("Create Jira swarm", use_container_width=True)
    do_slack = col3.button("Share to Slack #sec-war-room", use_container_width=True)

    if do_tf:
        st.toast("Terraform change plan drafted – awaiting approval.")
        with st.modal("Terraform plan preview"):
    ...
    if do_slack:
        ...
        with st.modal("Slack message draft"):
